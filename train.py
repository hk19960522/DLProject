...
def get_parser():
    ...
    parser.add_argument(
        '--using_cuda', default=True, type=ast.literal_eval)
    ...
    parser.add_argument(
        '--data_set', default=0, type=int,
        help='Set the value to 0~5 for test1, univ, zara1, zara2, eth, hotel')
    parser.add_argument(
...
def set_arg(p):
    ...
    logging.debug(f'{p.save_dir}, {p.model_dir}, {p.config}')
...
def load_arg(p):
    ...
    if os.path.exists(p.config):
        ...
        for k in default_arg.keys():
            if k not in key:
                logging.ERROR('WRONG ARG: {}'.format(k))
                try:
                    assert (k in key)
                except:
                    s = 1
